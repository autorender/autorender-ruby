...
Most of the SDK is generated code. Modifications to code will be persisted between generations, but may result in merge conflicts between manual patches and changes from the generator. The generator will never modify the contents of `lib/autorender/helpers/` and `examples/` directory.
...
require_relative "../lib/autorender"
...
```ruby
gem "autorender", path: "<path-to-repo>"
```
